...
initialize_df <- function(EVO2_npy_file, sequence_name) { # should be .npy file and both inputs in ""
  ...
  for (i in 2:length(seq_vec)) { # iterates from 1 to length of the sequence 
    ...
    log_likelihood[i] <- log(prob_matrix[i-1, base_index]) # calculates the log likelihood at row i
  }
...
# creates data frame with calculated entropy and log likelihood for 83_S1, 83_S2, 83_L, and 83_Stop
...
plot_log_likelihood <- function(gene_name, gene_df, start = 200, end = 300, col = "blue") {
  positions <- start:end
  plot(positions, gene_df$log_likelihood[positions],
       type = "l", col = "blue",
       xlab = "Nucleotide Position",
       ylab = "Log-Likelihood",
       main = sprintf("Log-Likelihood across positions %d–%d (%s)", start, end, gene_name))
}
plot_log_likelihood("83_S1", df_S1) # plotting log-likelihood for 83_S1
...
probability_correct_base <- function(gene, df_gene){ # input is a gene string such as "83_S1" and corresponding df 
  ...
  df_genes <- data.frame(variant = # makes a data frame with the first column seq names 
                           ...
                           names(dna_set),stringsAsFactors = FALSE)
  df_genes$sequence <- seqs # adds the sequence column to the genes df
  desired_seq <- seqs[gene] # gets the desired sequence of the gene
  seq_vec <- strsplit(desired_seq, split = "")[[1]] # turns the sequence into a list of single nucleotides
  correct_count <- 0
...
probability_correct_base("83_S1", df_S1) # probability EVO2 choosing correct nucleotide for "83_S1"
probability_correct_base("83_S2", df_S2)# probability EVO2 choosing correct nucleotide for "83_S2"
probability_correct_base("83_L", df_L)# probability EVO2 choosing correct nucleotide for "83_L"
probability_correct_base("83_Stop", df_Stop)# probability EVO2 choosing correct nucleotide for "83_Stop"
...
# CALCULATING CORRECT BASES FOR OTHER GENES
df_S2 <- initialize_df("input_83_S2_logits.npy", "83_S2") # save data from S2 prob matrix
df_L <- initialize_df("input_83_L_logits.npy", "83_L") # save data from S1 prob matrix
df_Stop <- initialize_df("input_83_Stop_logits.npy", "83_Stop") # save data from S1 prob matrix
