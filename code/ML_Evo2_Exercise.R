...
setwd("~/Desktop/Relman_lab/EVO2_Exercise/raw_data")
...
initialize_df <- function(EVO2_npy_file, sequence_name) { # should be .npy file and both inputs in ""
  ...
  for (i in 2:length(seq_vec)) { # iterates from 2 to length of the sequence
    ...
    log_likelihood[i-1] <- log2(prob_matrix[i-1, base_index]) # calculates the log likelihood at row i
  }
...
plot_entropy <- function(gene_name, gene_df, start = 200, end = 300, col = "blue") {
  EVO2_rows <- (start - 1):(end - 1)  # account for pos 1 assoc w/ nuc 2 from EVO2 data
...
probability_correct_base <- function(gene, df_gene, dna_set){ # input is a gene string such as "83_S1" and corresponding df 
  ...
  for (i in 2:length(split_seq)){
    max_column <- which.max(df_gene[i-1, 1:4])
...
positions <- (199:299) # to account for EVO2 starting at pos 2
